...
# %% [markdown]
# #### 2.2.1. Calculate firing rate for each sweep
...
for sweep in channels_data_frame_1.columns: 
    ...
    spikes_in_sweep_tmp = np.array([p for i, p in enumerate(peaks_1d_clean) if pseudo_sweep_concatenated[peaks_1d_clean[i]] == int(sweep)])

    #Get firing rate for the current sweep
    sweep_length_s_tmp = len(channels_data_frame_1.loc['Channel B', sweep]) * dt_1 / 1000 #in seconds
...
plt.scatter(seal_resistance, sweep_firing_rate)
...
# %% [markdown]
# #### 2.2.2. Calculate firing rate for time bins
...
# %%
n_bins = 100
time_window_s = len(pseudo_sweep_concatenated) / 1000 * dt_1 / n_bins # divide the length of recording in 100 chunks (1-2s each)
...
for window in range(n_bins): 
    ...
    # Take the spikes that belong to the current sweep
    spikes_in_window_tmp = np.array([p for p in peaks_1d_clean if ((time_window_samples*window) < p < (time_window_samples*(window+1)))])
